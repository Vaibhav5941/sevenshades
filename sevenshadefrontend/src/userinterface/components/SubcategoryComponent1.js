...
export default function SubcategoryComponent1(props)
{
  ...
  const sm_matches = useMediaQuery(theme.breakpoints.down('sm'));
    ...
    const showAllItems=()=>{
        return items.map((brand)=>{
          return <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:18}}>
                <div>
                  <img src={`${serverURL}${brand.icon}`} loading="lazy" style={{width:80,height:80}}/>
                </div>
          </div>
