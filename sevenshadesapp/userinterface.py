...
from django.shortcuts import render
from  sevenshadesapp.models import ProductDetail, MainCategory,SubCategory,Brand,Product,Banners,UsersAddress,Users
from sevenshadesapp.serializer import  UserAddressSerializer, ProductDetailGetSerializer, BannersSerializer,UserAddressGetSerializer,UserSerializer, MainCategorySerializer,SubCategoryGetSerializer,BrandSerializer,ProductGetSerializer,BannersSerializer
...
def Brand_List(request):
 ...
 try:
   if request.method=='POST': 
      ...
      maincategoryid=request.data['maincategoryid']
      product_list=Product.objects.all().filter(subcategoryid_id=subcategoryid,maincategoryid_id=maincategoryid)
      product_serializer_list=ProductGetSerializer(product_list,many=True) 
      #print("xcvbnm:",product_serializer_list.data)
 ...
 try:
   if request.method=='POST':
    ...
    if(user_serializer.is_valid()):
      ...
      return JsonResponse({"message":'User Registered Successfully',"status":True},safe=False)
    else:
...
def FetchUserAddress(request):
 ...
 try:
   if request.method=='POST':
     ...
     userAddress=UsersAddress.objects.all().filter(mobileno_id=mobileno)
     ...
     if(len(user_AddressSerializer.data)>0):
      return JsonResponse({"data":user_AddressSerializer.data,"status":True},safe=False)
        ...
        return JsonResponse({"data":[].data,"status":False},safe=False)
